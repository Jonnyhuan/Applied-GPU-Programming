...
__global__ void SAXPY_Kernel(float A, int iter_num, float *gpu_X, float *gpu_Y)
{
  const int i = blockIdx.x*blockDim.x + threadIdx.x;
  gpu_Y[i] = SAXPY(A, gpu_X[i], gpu_Y[i]);
}
...
int main(int argc, char *argv[])
{
  ...
  // looping host to device memcopy and kernal.
  for(int j=0; j<iter_num; j++)
  {
  	...
  	SAXPY_Kernel<<<(ARRAY_SIZE+TPB-1)/TPB, TPB>>>(A, iter_num, gpu_X, gpu_Y);
  ...
  cudaMemcpy(cpu_Y, gpu_Y, ARRAY_SIZE*sizeof(float),cudaMemcpyDeviceToHost);
